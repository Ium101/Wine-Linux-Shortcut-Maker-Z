...
class LinuxShortcutMaker:
    def __init__(self, root):
        ...
        self.root.minsize(480, 0)           # min width; height grows with content
        ...
        self.color_purple= "#CF9FFF"

        self.root.configure(bg=self.bg_color)

        # --- Executable type: "wine" (.exe via Wine) or "native" (Linux binary) ---
        ...
        self.category      = tk.StringVar(value="Utility")  # XDG desktop category key
        ...
        btn_style = {
            ...
            "activebackground": "#555963", "activeforeground": "white",
            "relief": "ridge", "bd": 1
        }

        # ── Credits (bottom) ─────────────────────────────────────────────────
        self.lbl_credits = tk.Label(root, text=self.texts[self.lang]["credits"],
        ...
        self.btn_lang = tk.Button(frame_top, text=self.texts[self.lang]["lang_btn"],
        ...
        # ── Title / subtitle ─────────────────────────────────────────────────
        self.lbl_title = tk.Label(root, text=self.texts[self.lang]["title"],
                                  font=("Arial", 14, "bold"), bg=self.bg_color, fg="white")
        self.lbl_title.pack(pady=(0, 2))
        self.lbl_subtitle = tk.Label(root, text=self.texts[self.lang]["subtitle"],
        ...
        frame_type = tk.Frame(root, bg=self.bg_color)
        frame_type.pack(fill="x", padx=20, pady=(0, 8))

        self.lbl_type = tk.Label(frame_type, text=self.texts[self.lang]["type_label"],
                                 font=("Arial", 10, "bold"), bg=self.bg_color, fg=self.fg_color)
        self.lbl_type.pack(anchor="w")

        rb_style = {"bg": self.bg_color, "fg": self.fg_color,
                    "activebackground": self.bg_color, "activeforeground": "white",
                    "selectcolor": self.sec_bg, "relief": "flat", "cursor": "hand2"}

        self.rb_wine = tk.Radiobutton(frame_type, text=self.texts[self.lang]["type_wine"],
                                      variable=self.exe_type, value="wine",
                                      command=self.on_type_change, **rb_style)
        self.rb_wine.pack(anchor="w", padx=(16, 0))

        self.rb_native = tk.Radiobutton(frame_type, text=self.texts[self.lang]["type_native"],
                                        variable=self.exe_type, value="native",
                                        command=self.on_type_change, **rb_style)
        self.rb_native.pack(anchor="w", padx=(16, 0))

        # ── Step 1: executable ────────────────────────────────────────────────
        self.lbl_step1 = tk.Label(root, text=self.texts[self.lang]["step1_default"],
                                  font=("Arial", 10, "bold"), bg=self.bg_color, fg=self.fg_color,
        ...
        self.lbl_exe = tk.Label(frame_exe, text=self.texts[self.lang]["no_file"],
                                fg=self.color_gray, bg=self.bg_color, justify="left",
                                anchor="w")
        self.lbl_exe.pack(side="left", expand=True, fill="x")
        # Bind wraplength dynamically so it always fits the label's actual width
        self.lbl_exe.bind("<Configure>", lambda e: self.lbl_exe.config(wraplength=e.width - 4))

        self.btn_browse = tk.Button(frame_exe, text=self.texts[self.lang]["browse_wine"],
                                    command=self.selecionar_exe, **btn_style)
        self.btn_browse.pack(side="right")

        # ── Step 2: name ──────────────────────────────────────────────────────
        self.lbl_step2 = tk.Label(root, text=self.texts[self.lang]["step2"],
                                  font=("Arial", 10, "bold"), bg=self.bg_color, fg=self.fg_color)
        self.lbl_step2.pack(anchor="w", padx=20, pady=(12, 0))

        self.entry_nome = tk.Entry(root, font=("Arial", 11), bg=self.sec_bg,
                                   fg="white", insertbackground="white", relief="flat")
        self.entry_nome.pack(fill="x", padx=20, pady=5)

        # ── Step 3: icon ─────────────────────────────────────────────────────
        self.lbl_step3 = tk.Label(root, text=self.texts[self.lang]["step3"],
                                  font=("Arial", 10, "bold"), bg=self.bg_color, fg=self.fg_color)
        ...
        self.lbl_icone = tk.Label(frame_icone, text=self.texts[self.lang]["default_icon"],
                                  fg=self.color_gray, bg=self.bg_color, justify="left",
                                  anchor="w")
        ...
        self.btn_icon = tk.Button(frame_icone, text=self.texts[self.lang]["change_icon"],
                                  command=self.selecionar_icone_manual, **btn_style)
        self.btn_icon.pack(side="right")

        # ── Step 4: extra arguments ───────────────────────────────────────────
        self.lbl_args = tk.Label(root, text=self.texts[self.lang]["args_label"],
                                 font=("Arial", 10, "bold"), bg=self.bg_color, fg=self.fg_color)
        self.lbl_args.pack(anchor="w", padx=20, pady=(12, 0))

        self.entry_args = tk.Entry(root, font=("Arial", 10), bg=self.sec_bg,
                                   fg=self.color_gray, insertbackground="white", relief="flat")
        ...
        self.lbl_cat = tk.Label(root, text=self.texts[self.lang]["cat_label"],
                                font=("Arial", 10, "bold"), bg=self.bg_color, fg=self.fg_color)
        self.lbl_cat.pack(anchor="w", padx=20, pady=(12, 0))

        frame_cat = tk.Frame(root, bg=self.bg_color)
        frame_cat.pack(fill="x", padx=20, pady=5)

        # Build initial display labels from EN
        ...
        self._cat_display = tk.StringVar(value=self._cat_labels[0])  # "Utilities"

        self.opt_cat = tk.OptionMenu(frame_cat, self._cat_display, *self._cat_labels,
        ...
        # ── Create button ─────────────────────────────────────────────────────
        self.btn_create = tk.Button(root, text=self.texts[self.lang]["btn_create"],
    ...
    def _args_focus_in(self, _event):
        if self._args_placeholder_active:
            ...
            self.entry_args.config(fg="white")
    ...
    def on_type_change(self):
        ...
        # Show detection label but in orange to signal manual override
        self.lbl_step1.config(
            text=t["step1_wine"] if is_wine else t["step1_native"],
            fg=self.color_orange
        )

        # Keep the already-selected file; just re-run icon logic for the new type
        if self.caminho_exe:
            if is_wine:
            ...
            # No file yet — reset icon label to the appropriate default
            if is_wine:
                self.caminho_icone = "wine"
    ...
    def _detectar_tipo(self, caminho):
        ...
        # 1. Extension: .exe is unambiguously Windows
        ...
        # 2. MZ magic → Windows PE
        if header[:2] == b"MZ":
            return "wine"

        # 3. .sh extension → Linux shell script
        if lower.endswith(".sh"):
            return "native"

        # 4. Shebang → script interpreted natively on Linux
        if header[:2] == b"#!":
            return "native"

        # 5. ELF magic (7f E L F) → native Linux/Unix binary
        if header[:4] == b"\x7fELF":
            return "native"

        # 6. Default: treat as native (safer than assuming Wine)
        return "native"
    ...
    def selecionar_exe(self):
        ...
        if caminho:
            ...
            # .sh files are always native regardless of current mode
    ...
    def buscar_icone_nativo(self, nome_binario):
        """
        Heuristic: check common icon directories for a file whose name contains
        the binary name (case-insensitive). If found, pre-fill the icon field.
        """
        ...
        for d in search_dirs:
            ...
            for f in p.iterdir():
                if nome_lower in f.name.lower() and f.suffix.lower() in (".png", ".svg", ".xpm", ".ico"):
                    ...
                    return
        # Nothing found — leave empty so user can pick manually
        self.caminho_icone = ""
        self.status_icone  = "default"
        self.lbl_icone.config(
    ...
    def _refresh_cat_menu(self):
        ...
        # Find current key's label in new language
    ...
    def criar_atalho(self):
        ...
        if is_wine and not is_shell_script:
            ...
            # Native binary or .sh script — run directly (or via sh for scripts)
            args_part = f" {extra_args}" if extra_args else ""
            if is_shell_script:
